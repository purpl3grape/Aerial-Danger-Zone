...
public class PlayerBase : CharacterBase
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.A) || Input.GetKey(KeyCode.LeftArrow)) {       InputDirection -= Vector2.right; }
        else if (Input.GetKey(KeyCode.D) || Input.GetKey(KeyCode.RightArrow)) { InputDirection += Vector2.right; }
        ...
        {
            if (Time.time - lastTimeSinceFiredShot > FireRate)
            {
                Shoot();
                lastTimeSinceFiredShot = Time.time;
            }
        }
